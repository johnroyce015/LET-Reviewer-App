...
async function loadAvailableQuizzes(course) {
    ...
    // Colors and icons matching your Neo-Brutalist UI palette
    const colors = ['#C4B5FD', '#FDE68A', '#6EE7B7', '#FCA5A5'];
    const icons = ['📐', '✍️', '🌍', '🔬', '💻'];

    // HTML Injection matching your gorgeous Figma layout designs perfectly
    categories.forEach((cat, index) => {
        ...
        card.style.cssText = 'border: 2px solid #111827; display: flex; justify-content: space-between; align-items: center; padding: 15px; margin-bottom: 12px; box-shadow: 4px 4px 0px #111827; border-radius: 12px;';
        
        card.innerHTML = `
            <div style="display: flex; align-items: center; gap: 15px; flex-grow: 1;">
                <div class="subject-icon" style="font-size: 20px; background: #fff; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border: 2px solid #111827; border-radius: 8px;">
                    ${cardIcon}
                </div>
                <div>
                    <h4 style="margin:0; font-size: 16px; font-weight: 900; color: #111827;">${cat.category_name}</h4>
                    <div style="font-size: 12px; color: #4B5563; font-weight: 700; margin-top: 2px;">
                        <i class="fa-solid fa-layer-group"></i> ${cat.exam_level ? cat.exam_level : 'General Education'} Level
                    </div>
                </div>
            </div>
            <button class="start-quiz-btn" data-category="${cat.category_name}" style="background: ${cardColor}; color: #111827; border: 2px solid #111827; padding: 8px 14px; border-radius: 8px; font-weight: 900; font-size: 12px; cursor: pointer; box-shadow: 2px 2px 0px #111827; margin: 0; white-space: nowrap;">
                Review ➡️
            </button>
        `;
    ...
    // Event Delegation handles the clicks dynamically without throwing scope errors!
    container.addEventListener('click', (e) => {
        const btn = e.target.closest('.start-quiz-btn');
        if (btn) {
            const categoryName = btn.getAttribute('data-category');
            localStorage.setItem('activeQuizCategory', categoryName);
            window.location.href = 'quiz.html'; // Redirects instantly to your active exam room
        }
    });
}